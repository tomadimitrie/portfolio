...
export default class Particles {
  ...
  show = (time = 1.0) => {
    ...
    TweenLite.fromTo(
      (this.object3D.material as any).uniforms.uDepth,
      time * 1.5,
      { value: 40.0 },
      { value: 4.0 }
    );
  ...
  hide = (_destroy, time = 0.8) => {
    return new Promise((resolve, reject) => {
      ...
      TweenLite.to((this.object3D.material as any).uniforms.uDepth, time, {
        value: -20.0,
        ease: Quad.easeIn,
      });
      TweenLite.to((this.object3D.material as any).uniforms.uSize, time * 0.8, {
        value: 0.0,
      });
